...
public class IssueApiController {
    ...
    public ApiResponse<List<IssueResponse>> list(IssueSearchCondition condition) {
        return ApiResponse.success(issueService.getIssueList(condition));
    }

    @GetMapping("/{issueId}")
    public ApiResponse<IssueResponse> detail(@PathVariable UUID issueId) {
        return ApiResponse.success(issueService.getIssue(issueId));
    }

    @PostMapping
    public ApiResponse<IssueResponse> create(@Valid @RequestBody IssueRequest request) {
        return ApiResponse.success("이슈가 등록되었습니다.", issueService.createIssue(request));
    }
